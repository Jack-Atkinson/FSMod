	...
	if (uuid_str) {
		...
		if ((session = switch_core_hash_find(session_manager.session_table, uuid_str))) {
			...
			if (switch_test_flag(session, SSF_DESTROYED)) {
				...
				switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, uuid_str, SWITCH_LOG_ERROR, "%s %s Read lock FAIL, perform_force_locate no try lock\n",
								  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel));
				// gqp modify end
#endif
			} else {
				...
				/*switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, uuid_str, SWITCH_LOG_ERROR, "%s %s Read lock ACQUIRED\n",
								  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel));*/
				switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, uuid_str, SWITCH_LOG_ERROR, "%s %s Read lock ACQUIRED, perform_force_locate result=%d\n",

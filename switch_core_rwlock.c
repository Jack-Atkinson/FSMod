	...
	if (session->rwlock) {
		if (switch_test_flag(session, SSF_DESTROYED) || switch_channel_down_nosig(session->channel)) {
			...
			/*switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, switch_core_session_get_uuid(session), SWITCH_LOG_ERROR, "%s %s Read lock ACQUIRED\n",
							  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel));*/
			switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, switch_core_session_get_uuid(session), SWITCH_LOG_ERROR, "%s %s Read lock ACQUIRED, result=%d\n",
							  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel), status);
			// gqp modify end
#endif
		}
	}
...
#endif
{
	...
	if (session->rwlock) {
		if (switch_test_flag(session, SSF_DESTROYED) || switch_channel_get_state(session->channel) >= CS_DESTROY) {
			...
			/*switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, switch_core_session_get_uuid(session), SWITCH_LOG_ERROR, "%s %s Read lock FAIL\n",
							  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel));*/
			switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, switch_core_session_get_uuid(session), SWITCH_LOG_ERROR, "%s %s Read lock FAIL, read_lock_hangup no try lock\n",
							  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel));
			// gqp modify end
#endif
		} else {
			...
			/*switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, switch_core_session_get_uuid(session), SWITCH_LOG_ERROR, "%s %s Read lock ACQUIRED\n",
							  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel));*/
			switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, switch_core_session_get_uuid(session), SWITCH_LOG_ERROR, "%s %s Read lock ACQUIRED, read_lock_hangup result=%d\n",
							  switch_core_session_get_uuid(session), switch_channel_get_name(session->channel), status);
			// gqp modify end
#endif
		}
